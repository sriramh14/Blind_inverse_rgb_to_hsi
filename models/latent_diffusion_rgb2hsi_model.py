...
class HSIDecoder(nn.Module):
    ...
    def forward(self, latent: torch.Tensor) -> torch.Tensor:
        x = self.level3(self.from_latent(latent))
        x = self.level2(self.up2(x, (x.shape[-2] * 2, x.shape[-1] * 2)))
        x = self.level1(self.up1(x, (x.shape[-2] * 2, x.shape[-1] * 2)))
